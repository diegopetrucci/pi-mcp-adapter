...
function normalizeHashEnvironment(value: HashEnvironment | undefined): {
  environment: NodeJS.ProcessEnv;
  defaultCwd?: string;
} {
  if (typeof value === "string") return { environment: process.env, defaultCwd: value };
  return { environment: value ?? process.env };
}

export function computeServerHash(definition: ServerEntry, environmentOrCwd: HashEnvironment = process.env): string {
...
export function isServerCacheValid(
  ...
  environmentOrCwd: HashEnvironment = process.env,
): boolean {
