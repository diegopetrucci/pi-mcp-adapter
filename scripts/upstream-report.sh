...
#
# This is a read-only reporting helper for pi-mcp-adapter (see
# docs/UPSTREAM-SYNC.md §7). It prints ahead/behind counts vs upstream/main,
# fork/upstream version signals, and a `git cherry`/patch-id heuristic of which
# fork commits may already be reflected upstream.
#
# That heuristic can be wrong (squashed, split, or conflict-edited commits will
# not match). It must NEVER be used to mark an upstream change as adopted, skip
# ledger bookkeeping, or skip review during an intake.
#
# The single source of truth for what has actually been integrated is:
#   - the git DAG (fork merge/squash-import history), and
#   - .upstream-ledger.jsonl (the intake ledger).
# Any disagreement between this script's output and the ledger/DAG is resolved
# in favor of the ledger/DAG, always. See docs/UPSTREAM-SYNC.md §3 and §7.
#
# This script makes NO repository worktree changes. It may update local remote
# refs via `git fetch upstream`. If `upstream` is absent or fetch fails, it
# degrades gracefully with a clear message and exits 0 when possible.
#
# Usage:
#   ./scripts/upstream-report.sh
...
banner() {
  echo "============================================================"
  echo " pi-mcp-adapter upstream-report: SIGNAL ONLY — NOT AUTHORITATIVE"
  echo " Source of truth: git DAG + .upstream-ledger.jsonl"
  echo " (see docs/UPSTREAM-SYNC.md §3 and §7)"
  echo "============================================================"
}
...
if [ "$FETCH_STATUS" -ne 0 ]; then
  echo "$FETCH_OUTPUT" >&2
  echo "Warning: 'git fetch upstream' exited non-zero (for example due to a" >&2
...
if [ -f package.json ]; then
  FORK_VERSION=$(node -e "const fs=require('fs');console.log(JSON.parse(fs.readFileSync('package.json','utf8')).version)" 2>/dev/null || echo "unknown")
  echo "Fork package.json version: $FORK_VERSION"
else
  echo "No package.json found in current directory."
fi

LATEST_UPSTREAM_TAG=$(git tag --merged "$UPSTREAM_REF" --list 'v*' --sort=-v:refname 2>/dev/null | head -n1)
if [ -z "$LATEST_UPSTREAM_TAG" ]; then
  LATEST_UPSTREAM_TAG=$(git tag --list 'v*' --sort=-v:refname 2>/dev/null | head -n1)
fi
if [ -n "$LATEST_UPSTREAM_TAG" ]; then
  echo "Latest known upstream-reachable tag: $LATEST_UPSTREAM_TAG"
else
  echo "No v* tags found to determine an upstream version signal."
fi
...
echo "See docs/UPSTREAM-SYNC.md §3 for why this can be wrong."
echo
