...
describe("updateStatusBar", () => {
  it("dims the footer and shows connected names", () => {
  ...
  it("shows connected servers by name", () => {
    const setStatus = vi.fn();
    const state = createState({ setStatus });
    ...
    expect(setStatus).toHaveBeenCalledWith("mcp", "MCP: 1/1 servers, demo");
  });

  it("keeps the dim theme when a theme is available", () => {
    const setStatus = vi.fn();
  ...
  it("preserves the TLH footer when the icon setting is explicitly enabled", () => {
    const setStatus = vi.fn();
